...
# Note: the delay spread sets frequency selectivity, not the per-subcarrier
# spatial rank, so the singular-value distribution is essentially independent
# of it (verified: DS=30 ns and DS=300 ns give the same spectrum). The lever for
# effective rank is the channel model / angle spread (CDL-B vs C vs D).


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--ds", type=float, default=300.0, help="RMS delay spread (ns)")
    ...
    for i in range(n_samples):
        ch = CDLChannel("CDL-C", delay_spread_ns=args.ds, max_doppler_hz=10,
    ...
    print(f"{N_TX}T{N_RX}R CDL-C, DS={args.ds:g} ns, {n_samples} samples, "
          f"{N_RX} singular values")
